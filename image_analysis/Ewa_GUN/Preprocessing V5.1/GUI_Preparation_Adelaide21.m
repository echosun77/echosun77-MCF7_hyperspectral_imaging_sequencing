 function GUI_Preparation_Adelaide21


%%  Create and then hide the UI as it is being constructed.
f = figure('Visible','on','units','normalized','outerposition',[0 0 1 1],...
    'Tag','fHandle','Name','GUI Preparation ver. 1.0','Numbertitle','off');
...
menuMasksH =  uimenu(menuOpenH,'Label','Masks','Enable','on',...
...
uipanelMasksH = uipanel('Title','Masks','FontSize',12,...
             'Position',[.6666 .0 .3333 1],...
             'Parent',uipanelParH,...
             'BorderWidth',3);
...
popupCosmicH = uicontrol('Style','popup',...
                ...
                'String',{'no cosmic rays removal','Fast Cosmic Rays Removal'},...
                'Units','normalized',...
...
popupWaveletWCH = uicontrol('Style','popup',...
                'Parent',uipanelSmoothWCH,...
                'String',{'wavelet'},...
                'Units','normalized',...
                'Position',[0.4 0.8 0.6 0.2]);
...
sliderDenoiseWCH = drawSlider(1000,500,10000,[0.05 0.5 0.9 0.25],uipanelSmoothWCH, 1,1000);
...
textSmoothParamWCH = uicontrol('Style','text','String','Smoothing parameter: 1000','FontSize',12,...
    'Parent',uipanelSmoothWCH,...
    'Units','normalized',...
    'HorizontalAlignment','left',...
    'Position',[0 0.2 0.35 0.1]);
setappdata(f,'textSmoothParamWCH',textSmoothParamWCH);
...
popupWaveletCH = uicontrol('Style','popup',...
                'Parent',uipanelSmoothCH,...
                'String',{'no wavelet','wavelet'},...
                'Units','normalized',...
                'Position',[0.4 0.8 0.6 0.2]);
setappdata(f,'popupWaveletCH', popupWaveletCH)

sliderDenoiseCH = drawSlider(1000,500,10000,[0.05 0.5 0.9 0.25],uipanelSmoothCH, 1,1000);
...
textSmoothParamCH = uicontrol('Style','text','String','Smoothing parameter: 1000','FontSize',12,...
    'Parent',uipanelSmoothCH,...
    'Units','normalized',...
    'HorizontalAlignment','left',...
    'Position',[0 0.2 0.35 0.1]);
setappdata(f,'textSmoothParamCH',textSmoothParamCH);
...
buttonUnmixTabH = uicontrol(uipanelUnmixTabH,'Style','pushbutton',...
             'String','Generate table for unmxing',...
             'Units','normalized',...
             'Position',[0 0 1 1],...
...
if ~isequal(filenameMask,0)
    ...
    set(uipanelMasksH,'Title',['Masks | ',num2str(length(filenameMask)), ' files'])
end
end
...
structConst.FLUOROLOG = tableF(:,1);
nChannel = length(tableF(:,1));

% USER REQUESTED TO REMOVE RAYS 1 or 0
remRays = (get(getappdata(figH,'popupCosmicH'),'Value')==2);
...
smoothParamWC = get(getappdata(figH,'sliderDenoiseWCH'),'Value');

% USER REQUESTED TO SMOOTH CELLS 1 or 0
smoothC = (get(getappdata(figH,'popupWaveletCH'),'Value')==2);
smoothParamC = get(getappdata(figH,'sliderDenoiseCH'),'Value');

% checking if we have water and calibration fluid

if length(listWater)==0
     msgTemplate([],[],'Choose water file!','Error')
          return
end
if length(listCalibration)==0
...
if ~isempty(strfind(listWater,'_prepared_smoothed')) %avoiding operations if water prepared
...
sens = structConst.SENS
    try
        if exist('specImage','var')==1 %old structure
            ...
            tempTimeGainQE = ones(size(HAC_Image.imageStruct.data));

            for i=1:nChannel
                tempTimeGainQE(:,:,i)=tempTimeGainQE(:,:,i)...
                    *str2double(HAC_Image.imageStruct.protocol.channel(i).exposure)... %str2double(HAC_Image.imageStruct.protocol.channel(i).emGain) is always equal to 1
                    *structConst.QENEW(i)...
                    ./structConst.FLUOROLOG(i);
            ...
            tempWater = (HAC_Image.imageStruct.data - bias)*sens./tempTimeGainQE;
    ...
    for iChannel = 1:nChannel
        if fileStruct==1
            ...
            tempWater(:,:,iChannel) = imresize(denoise(imresize(squeeze(tempWater(:,:,iChannel)),...
                [1024, 1024]),smoothParamWC),sizeTW);
              progressbar(iChannel/(nChannel+0.01))
            disp(['Channel: ',num2str(iChannel),' smoothed.'])
        else
            tempWater(:,:,iChannel) = denoise(tempWater(:,:,iChannel),smoothParamWC);
              progressbar(iChannel/(nChannel+0.01)) 
             disp(['Channel: ',num2str(iChannel),' smoothed.'])
        end
    end
...
if ~isempty(strfind(listCalibration,'_prepared_smoothed')) %avoiding operations if water prepared
    ...
    try
        ...
        if exist('specImage','var')==1 %old structure
            ...
            tempCalibration = (HAC_Image.imageStruct.data - bias)*sens./tempTimeGainQE;
        end
    catch
        msgTemplate([],[],{'No image found in: ';listCalibrationFull{iFile}},'Error')
    end  
    ...
    for iChannel = 1:nChannel
        if fileStruct==1
            ...
            tempCalibration(:,:,iChannel) = denoise(tempCalibration(:,:,iChannel),smoothParamWC);
            progressbar(iChannel/(nChannel+0.01)) 
            disp(['Channel: ',num2str(iChannel),' smoothed.'])
        end
    end
...
for iFile=1:length(listCellFull)
    ...
    try
        if exist('specImage','var')==1 %old structure
            ...
            tempCell = (HAC_Image.imageStruct.data - bias)*sens./tempTimeGainQE;
        end
    catch
        msgTemplate([],[],{'No image found in: ';listCellFull{iFile}},'Error')
    end
    
    % final operations/flattening
%     for iChannel = 1:nChannel
%         tempCell(:,:,iChannel) = (tempCell(:,:,iChannel)-tempWater(:,:,iChannel))./(tempCalibration(:,:,iChannel)-tempWater(:,:,iChannel));  %% it should be minus first then smooth
%     end
    
     tempCell(:,:,1:nChannel) = (tempCell(:,:,1:nChannel)-tempWater(:,:,1:nChannel))...
        ./tempCalibration(:,:,1:nChannel);
    progressbar('Saving...');
        ...
        for iChannel = 1:nChannel
            if fileStruct==1
                ...
                tempCell(:,:,iChannel) = imresize(denoise(imresize(squeeze(tempCell(:,:,iChannel)),...
                    [1024, 1024]),smoothParamC),sizeTW);
                progressbar(iChannel/(nChannel+0.01))
                disp(['Channel: ',num2str(iChannel),' smoothed.'])
            else
                tempCell(:,:,iChannel) = denoise(tempCell(:,:,iChannel),smoothParamC);
                progressbar(iChannel/(nChannel+0.01)) 
                disp(['Channel: ',num2str(iChannel),' smoothed.'])
            end
        end
...
% checking if for every file we have mask 
...
for iFile=1:length(listCellName)
    ...
    [tempData, ~] = readFile(strcat(pathnameCell,listCellName{iFile}));
    tempMask = im2bw(imread(strcat(pathnameMask,listMaskName{selectedCell})));
...
% GETTING NECESSAARY DATA FROM GUI
listCellsH = getappdata(figH,'listCellsH');
listCellName = get(listCellsH,'string');
listMasksH = getappdata(figH,'listMasksH');
listMaskName = get(listMasksH,'string');
...
for iFile=1:length(listMaskName)
    % loading cell file
    [tempCell, ~] = readFile(strcat(pathnameCell,listCellName{iFile}));
    ...
    % loading mask
    tempMask = im2bw(imread(strcat(pathnameMask,listMaskName{iFile})));
...
if get(getappdata(figH,'popupCosmicH'),'Value')==2
...
if (get(getappdata(figH,'popupWaveletWCH'),'Value')==1 & filePanel==1) || (get(getappdata(figH,'popupWaveletCH'),'Value')==2 & filePanel==2 )
% getting number of channel
channel = str2double(inputdlg('Enter Channel:',...
             'Select Channel', [1 50]));
    if isempty(channel)
        ...
        switch filePanel
            case 1
                 ...
                 sliderVal = get(getappdata(figH,'sliderDenoiseWCH'),'Value');
            case 2
                 fileType = {'listCellsH'};
                 fileFolder = {'pathnameCell'};
                 sliderVal = get(getappdata(figH,'sliderDenoiseCH'),'Value');
        end
        listSel = get(getappdata(figH,fileType{get(hObject,'Value')}),{'string','value'});
        if iscell(listSel{1})
            listSel =  listSel{1}(listSel{2});
            ...
            [tempData, fileStruct] = readFile(listSelFull{1});
             ...
             if fileStruct==1 %resizing of image for denoising if size is not 2^n x 2^n
                ...
                try
                    % obtaining results
                    im = denoise(tempData,...
                        sliderVal); % value of slider
                    % downsising in case of old structure
                    ...
                    title(ax4,['Parameter: ',num2str(sliderVal)])
...
% Callback for changing Text in Window Size                      
set(getappdata(figH,'textSmoothParamWCH'),'String',...
    ['Smoothing parameter: ',num2str(get(getappdata(figH,'sliderDenoiseWCH'),'Value'))]);
end

function sliderDenoiseCH_callback(hObject,callbackdata,figH)
% Callback for changing Text in Window Size                      
set(getappdata(figH,'textSmoothParamCH'),'String',...
    ['Smoothing parameter: ',num2str(get(getappdata(figH,'sliderDenoiseCH'),'Value'))]);
end
