...
options.null = 0;
% h = getoptions(options, 'filter', '9-7'); % ALINE
% separable = getoptions(options, 'separable', 0); % ALINE
% h = '9-7'; % ALINE
separable = 0; % ALINE
...
%    end
% end
h = [1.586134342 -.05298011854 -.8829110762 .4435068522 1.149604398]; % ALINE -> 9-7

% if d==1 && size(x,1)==1 % ALINE: commented
...
if d==2 && separable==1 % sometimes separable != 1?
    options.ti = 0;
    if ti==1
...
if ti==0
    ...
    if dir==1 && d==1
        x = repmat(x,[1 1 nJ+1]);
    elseif dir==1 && d==2
        x = repmat(x,[1 1 nJ+1]);%repmat(x,[1 1 3*nJ+1]);
    end
    for j=jlist
        dist = 2^(Jmax-j);
        if d==1
