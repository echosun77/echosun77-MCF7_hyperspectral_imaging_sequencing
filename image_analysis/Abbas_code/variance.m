...
if isequal(w,0) || isequal(w,1)
    ...
    if ~omitnan
        if w == 0
            % The unbiased estimator: divide by (n-1).  Can not do this
            % when n == 0 or 1.
            if n > 1
                denom = n - 1;
            else
        ...
        n = sum(~isnan(x), dim);
        if w == 0
            % The unbiased estimator: divide by (n-1).  Can not do this when
            % n == 0 or 1
            denom = n-1;
            denom(n == 1) = 1;
            denom(n == 0) = 0;  
            ...
            % The biased estimator: divide by n.
            denom = n; % n==1 => we will return zeros
        end
    ...
    if ~omitnan
        ...
        % replicate it out along the non-working dims to match the size of X
        ...
        % Do not omit NaNs caused by computation (not missing data)
